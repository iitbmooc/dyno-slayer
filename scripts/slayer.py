...
def alert(msg):
    ...
    """
    logging.warn(msg)
    # return 0, because this script completed its job successfully.
    return 0
...
def main(app, settings):
    ...
    def process_app_logs(app, queue):
        ...
        for line in app.logs(source="app", ps="web", tail=True):
            ...
            line_cnt += 1
            if line_cnt % 10 == 0:
                logging.debug('read {} lines'.format(line_cnt))
    ...
    try:
        ...
        # clean up subprocess before continuing...
        if p.is_alive():
            logging.debug('terminating log tail process')
            p.terminate()
            p.join()
